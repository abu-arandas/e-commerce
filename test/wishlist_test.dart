...
void main() {
  ...
  test('the badge and the page agree when a piece leaves the catalogue',
      () async {
    final product = catalog.products.first;
    await wishlist.toggle(product);
    await settle();
    ...
    expect(wishlist.savedProducts.length + wishlist.unavailableCount.value,
        wishlist.count,
        reason: 'shown + unavailable must always equal saved');
  });

  test('a saved id with no product anywhere is reported, not dropped silently',
      () async {
    ...
    expect(wishlist.unavailableCount.value, 1,
  ...
  group('signing out', () {
    ...
    test('empties the wishlist rather than just stopping the refresh',
        () async {
      final auth = Get.find<AuthController>();
      auth.user.value = const AppUser(id: 'shopper-1', email: 'a@example.test');
      await settle();
      ...
      expect(wishlist.count, 0, reason: 'the badge must not survive sign-out');
      expect(wishlist.savedProducts, isEmpty);
      expect(wishlist.unavailableCount.value, 0);
    });

    test('the next account does not inherit the previous one\'s saves',
        () async {
      final auth = Get.find<AuthController>();
      auth.user.value = const AppUser(id: 'shopper-1', email: 'a@example.test');
      await settle();
      ...
      auth.user.value = null;
      await settle();
      auth.user.value = const AppUser(id: 'shopper-2', email: 'b@example.test');
      await settle();

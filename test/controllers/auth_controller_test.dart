...
void main() {
  ...
  group('AuthController signIn Tests', () {
    test(
      'Happy Path: Returns true and sets user on success (fallback profile handling)',
      () async {
        final mockUser = User(
          id: 'user-123',
          appMetadata: {},
    ...
    test(
      'Invalid Credentials: Returns false and sets error when user is null',
      () async {
    ...
    test(
      'AuthException: Returns false and sets specific error message',
      () async {
        when(
          () => mockAuthClient.signInWithPassword(
            email: 'test@example.com',
    ...
    test(
      'Generic Exception: Returns false and sets generic error message',
      () async {
        when(
          () => mockAuthClient.signInWithPassword(
            email: 'test@example.com',
        ...
        expect(
          ...
          'Sign-in failed: Exception: Network error',
        );
        expect(authController.user.value, isNull);
      },
    );
